...
$gemv_exec_path -d $2 -re --stdout-file=simout --stderr-file=simerr $config_path --cpu-type=$cpu_type --caches -n $num_procs -c "$bench_home/$bench" -o "$options" --cpu-type=$cpu_type --caches --vul_analysis=yes --vul_params=params.in --output=$2/result